...
class DisciplinaController extends Controller
{
    ...
    {
        $disciplinas = Disciplina::where('escola_id', $escola->id)
            ->select('id','id_externo','nome','codigo_externo')
            ->orderBy('id')
            ->get();

        if ($req->boolean('debug')) {
            $saida = $disciplinas->map(fn($d) =>
                "{$d->id} - {$d->nome} - {$d->codigo_externo}"
            )->implode("\n");

            return response($saida, 200)->header('Content-Type','text/plain; charset=utf-8');
        }

        return response()->json($disciplinas, 200, [], JSON_UNESCAPED_UNICODE);
    }
